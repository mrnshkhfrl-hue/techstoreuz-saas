...
export default function SuperAdminAuthWrapper({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    try {
      ...
      const superAdminIds = (process.env.NEXT_PUBLIC_SUPERADMIN_IDS || "7949519588").split(",").map(s => s.trim());
      
      if (superAdminIds.includes(String(userId))) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-black text-white p-4 text-center">
        <div className="p-8 max-w-sm w-full rounded-[28px] bg-white/[0.04] backdrop-blur-2xl border border-white/[0.08] space-y-3">
          ...
          <p className="text-xs text-white/50 max-w-sm mx-auto">Эта страница доступна только создателю платформы (Супер-админу).</p>
        </div>
      </div>
    );
  }
